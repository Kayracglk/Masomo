...
MonoBehaviourPunCallbacks, IPunObservable, IPunOwnershipCallbacks 
        ...
        void Start() {
            ...
            if (PhotonNetwork.IsConnected && photonView.IsMine) {
                // İstenilen silah prefabını instantiate et
                GameObject weapon = PhotonNetwork.Instantiate(startingWeaponPrefab.name, Vector3.zero, Quaternion.identity);
                if (RightGrabber != null && weapon != null) {
                    Grabbable grabbableWeapon = weapon.GetComponent<Grabbable>();
                    if (grabbableWeapon != null) {
                        RightGrabber.GrabGrabbable(grabbableWeapon);
                        print("Grabbed Weapon : " + weapon.name);
                    }
                }
            }
        }
        ...
        void toggleObjects(bool enableObjects) {
            ...
            Avatar.SetActive(enableObjects);
